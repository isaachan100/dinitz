...
class FlowNetwork:
    ...
    def compute_blocking_flow(self, graph: List[List[int]]) -> List[List[int]]:
        ...
        while len(stack) != 0:
            ...
            if (u == self.dest):
                delta = min([residual_capacities[stack[i]][stack[i+1]] for i in range(len(stack) - 1)])
    ...
    def compute_advancing_graph(self, capacities: List[List[int]]):
        ...
        residual_capacities = [[0 for _ in range(self.num_vertices)] for _ in range(self.num_vertices)]
